...
export function renderWelcome(fields: StaffMergeFields): StaffEmailContent {
  ...
  return {
    ...
    bodyHtml: `
      ...
      ${para(`We&apos;ve attached your <strong>Parent Welcome Packet</strong> to this email. Please take a few minutes to read it when you have the chance — it explains, in plain language, exactly how ABA services work, who will be part of ${child}&apos;s team, and the step-by-step journey from today through your child&apos;s first session. There are no surprises hidden in it, and nothing you&apos;ll be expected to already know.`)}
      ...
      ${para(`In the next day or two, you&apos;ll receive a second email from us with your <strong>Intake and Consent forms</strong> and a secure link where you can complete everything and upload your documents. Getting those back to us — complete and accurate the first time — is the single biggest thing you can do to help ${child} start sooner.`)}
