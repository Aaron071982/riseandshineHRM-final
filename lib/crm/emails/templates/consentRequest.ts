...
export function renderConsentRequest(fields: StaffMergeFields): StaffEmailContent {
  ...
  return {
    ...
    bodyHtml: `
      ...
      ${sectionRule()}
      ${para(`<strong>Please complete both forms and email the finished copies back to us</strong> (reply to this message or send them to <a href="mailto:${email}" style="color:${ACCENT};text-decoration:none;">${email}</a>). You can also include supporting documents with that email when you have them — for example:`)}
