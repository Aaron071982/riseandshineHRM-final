...
import type { ApplicationData, ApplicationFileField } from './types'

interface ApplicationStepFilesProps {
  data: ApplicationData
  setData: React.Dispatch<React.SetStateAction<ApplicationData>>
  onFileChange: (field: ApplicationFileField, file: File | null) => void
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-6">
        ...
        <div>
          <Label htmlFor="diplomaGed">
            High school diploma or GED * (PDF, JPG, PNG, HEIC, or WEBP, max 10MB)
          </Label>
          <div className="mt-2">
            <label
              htmlFor="diplomaGed"
            ...
            >
              ...
              <input
                ...
                type="file"
                accept=".pdf,.jpg,.jpeg,.png,.heic,.heif,.webp"
                onChange={(e) => handleFileChange('diplomaGed', e.target.files?.[0] || null)}
                className="hidden"
              />
            </label>
            {data.diplomaGed && (
              ...
              >
                <div className="flex items-center gap-3 flex-1 min-w-0">
                  <Paperclip className="h-4 w-4 text-primary flex-shrink-0" />
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-gray-900 truncate">{data.diplomaGed.name}</p>
                    <p className="text-xs text-gray-500">{(data.diplomaGed.size / 1024 / 1024).toFixed(2)} MB</p>
                  </div>
                </div>
                <button
                  type="button"
                  onClick={() => handleFileChange('diplomaGed', null)}
                  className="p-1 hover:bg-gray-100 rounded transition-colors"
                >
                  <X className="h-4 w-4 text-gray-400 hover:text-gray-600" />
                </button>
