...
export default function AddBillingForm() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await fetch('/api/admin/employees/billing', {
        method: 'POST',
      ...
      if (!res.ok) {
        setError(data.error || 'Failed to create billing profile')
        setLoading(false)
        return
      }
      router.push(`/admin/employees/billing/${data.id}`)
    } catch {
  ...
  return (
    <Card className="dark:bg-[var(--bg-elevated)] dark:border-[var(--border-subtle)]">
      <CardHeader>
        <CardTitle>Billing Team Member</CardTitle>
      </CardHeader>
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              ...
              <Input id="department" name="department" className="dark:bg-[var(--bg-input)] dark:border-[var(--border-subtle)]" />
            </div>
            <div className="space-y-2">
              <Label htmlFor="startDate">Start Date</Label>
              <Input id="startDate" name="startDate" type="date" className="dark:bg-[var(--bg-input)] dark:border-[var(--border-subtle)]" />
            </div>
            <div className="space-y-2">
