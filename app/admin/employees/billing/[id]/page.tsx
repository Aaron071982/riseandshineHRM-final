...
export default async function BillingDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <Link href="/admin/employees?type=BILLING" className="text-sm text-blue-600 dark:text-blue-400 hover:underline mb-2 inline-block">
          ← Back to Employees
        </Link>
        <h1 className="text-3xl font-bold text-gray-900 dark:text-[var(--text-primary)]">{profile.fullName}</h1>
        <p className="text-gray-600 dark:text-[var(--text-tertiary)]">Billing profile</p>
      </div>
      ...
      <Card className="dark:bg-[var(--bg-elevated)] dark:border-[var(--border-subtle)]">
        ...
        <CardContent className="space-y-2">
          ...
          {profile.department && <p><span className="text-gray-500 dark:text-[var(--text-tertiary)]">Department:</span> {profile.department}</p>}
          {profile.startDate && <p><span className="text-gray-500 dark:text-[var(--text-tertiary)]">Start date:</span> {formatDate(profile.startDate)}</p>}
      ...
      <StaffHoursLogSection employeeType="BILLING" referenceId={profile.id} />
      ...
      <Card className="dark:bg-[var(--bg-elevated)] dark:border-[var(--border-subtle)]">
        ...
        <CardContent>
          <EmployeeDeleteSection
            ...
            email={profile.email}
            deleteApiUrl={`/api/admin/employees/billing/${profile.id}/delete`}
            redirectHref="/admin/employees?type=BILLING"
          />
        </CardContent>
      </Card>
    </div>
