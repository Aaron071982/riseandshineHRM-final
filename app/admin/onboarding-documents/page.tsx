...
import BcbaOnboardingDocumentsAdmin from '@/components/admin/BcbaOnboardingDocumentsAdmin'
...
function OnboardingDocsError() {
  return (
    <div className="rounded-lg border-2 border-amber-200 bg-amber-50 dark:bg-[var(--status-warning-bg)] dark:border-[var(--status-warning-border)] p-6 text-center">
      <p className="font-semibold text-amber-900 dark:text-[var(--status-warning-text)]">
        Could not load onboarding documents
      </p>
      <p className="text-sm text-amber-700 dark:text-[var(--status-warning-text)] mt-2">
...
export default async function OnboardingDocumentsAdminPage() {
  ...
  let bcbaDocuments: Awaited<ReturnType<typeof prisma.bcbaOnboardingDocument.findMany>> = []
  try {
  ...
  try {
    bcbaDocuments = await prisma.bcbaOnboardingDocument.findMany({
      orderBy: [{ displayOrder: 'asc' }, { createdAt: 'asc' }],
    })
  } catch (error) {
    console.error('Admin bcba onboarding-documents: failed to load', error)
    bcbaDocuments = []
  }

  return (
    <div className="space-y-10">
      <OnboardingDocumentsAdmin initialDocuments={documents} />
      <BcbaOnboardingDocumentsAdmin initialDocuments={bcbaDocuments} />
    </div>
  )
}
