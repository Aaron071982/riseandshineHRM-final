...
import { isClientManagerAdmin, type SessionUser } from '@/lib/auth'
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // --- Client Management: auth expiring + weekly digest (client manager admins only) ---
    try {
      const adminUsers = await prisma.user.findMany({
        where: { role: 'ADMIN', isActive: true, email: { not: null } },
        select: { id: true, email: true, name: true, phoneNumber: true, role: true },
      })
      const cmsRecipients = adminUsers.filter((u) =>
        isClientManagerAdmin({
          id: u.id,
          email: u.email,
          name: u.name,
      ...
      const cmsEmails = cmsRecipients.map((u) => u.email!).filter(Boolean)
      if (cmsRecipients.length > 0) {
        const nowEt = new Date()
        const dayStart = startOfDay(nowEt)
        const thirty = new Date(nowEt.getTime() + 30 * 24 * 60 * 60 * 1000)

        const authExpiring = await prisma.crmClient.findMany({
          where: {
            status: 'ACTIVE',
            authorizationEndDate: { not: null, lte: thirty, gte: dayStart },
          },
        ...
        if (authExpiring.length > 0) {
          for (const admin of cmsRecipients) {
            await prisma.adminNotification
              .create({
                data: {
                  userId: admin.id,
                  type: 'CRM_AUTH_EXPIRING',
                  message: `${authExpiring.length} client authorization(s) expiring within 30 days.`,
                  linkUrl: makePublicUrl('/admin/clients'),
                },
              })
              .catch(() => {})
          }
          const listHtml = authExpiring
            .map(
              (c) =>
                `<li><a href="${makePublicUrl(`/admin/clients/${c.id}`)}">${c.firstName} ${c.lastName}</a> — ends ${c.authorizationEndDate?.toLocaleDateString('en-US', { timeZone: 'America/New_York' })}</li>`
            )
            .join('')
          const emailBody = `<!DOCTYPE html><html><body style="font-family:system-ui,sans-serif;padding:24px;"><h2>⚠️ Authorizations expiring</h2><ul>${listHtml}</ul></body></html>`
          for (const to of cmsEmails) {
            await sendGenericEmail(to, '⚠️ Authorization expiring — clients need review', emailBody).catch(
              () => {}
            )
          }
        }
        ...
        if (nowEt.getDay() === 1) {
          const weekStart = startOfWeek(nowEt, { weekStartsOn: 1 })
          const already = await prisma.activityLog.count({
            where: { action: 'CLIENT_WEEKLY_DIGEST', createdAt: { gte: weekStart } },
          })
          if (already === 0) {
            const noRbt = await prisma.crmClient.findMany({
              where: {
                status: 'ACTIVE',
                rbtAssignments: {
                  none: {
            ...
            const activeClients = await prisma.crmClient.findMany({
              where: { status: 'ACTIVE' },
              select: {
                id: true,
                firstName: true,
            ...
            const lowHours = activeClients.filter((c) =>
              hoursRunningLow({
            ...
            if (noRbt.length > 0 || lowHours.length > 0) {
              const html = `<!DOCTYPE html><html><body style="font-family:system-ui,sans-serif;padding:24px;">
                <h2>Weekly client digest</h2>
                <p><strong>Active with no RBT:</strong> ${noRbt.length}</p>
                <ul>${noRbt.map((c) => `<li>${c.firstName} ${c.lastName}</li>`).join('')}</ul>
                <p><strong>Hours running low:</strong> ${lowHours.length}</p>
                <ul>${lowHours.map((c) => `<li>${c.firstName} ${c.lastName}</li>`).join('')}</ul>
              </body></html>`
              for (const to of cmsEmails) {
                await sendGenericEmail(to, '📋 Weekly client digest — Rise & Shine', html).catch(() => {})
              }
            }

            const logUserId = cmsRecipients[0]?.id ?? adminUsers[0]?.id
            if (logUserId) {
      ...
      console.error('[staleness-alerts] CRM block:', crmErr)
    }
